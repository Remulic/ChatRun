...
const RunDate = dbModel.model('RunDate')
const Dynamic = dbModel.model('Dynamic')
const Discuss = dbModel.model('Discuss')
const Manger = dbModel.model('Manger')
/**************************************** */
/**
 * 新建用户-增
 * @param {用户名} name 
...
exports.mangerLogin = (name, psw, res)=>{
  let wherestr = { 'name': name,'psw': psw }

  Manger.find(wherestr, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
...
exports.searchUser = (data, res)=>{
  ...
  if(data === 'ChatRoom'){
    wherestr = {}
    let out = { '_id':1,'name':1, 'email': 1, 'time': 1,'sex':1}
    User.find(wherestr,out, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
    ...
    let out = {'_id':1,'name':1, 'email': 1, 'imgurl': 1,'time': 1,'sex':1}
    User.find(wherestr, out, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
 ...
 * @param {*} res 
 */
exports.findFriend = (uid, res)=>{
  let wherestr = { 'userID': uid, 'state': 0 }
  let out = {'friendID':1,'markname':1}
...
//删
exports.userDelete = (data, res)=>{
  let wherestr = { _id:data._id}
  let whererun = { userID:data._id}
  console.log(wherestr);
  User.findByIdAndRemove(wherestr, (err, result)=>{
  ...
  RunDate.deleteOne(whererun,(err,result)=>{
    // console.log(111);
  })
 ...
 */
// 更新数据的方法
exports.updateRunDate=(data,res ) => {
  let wherestr = {'_id':data.id}
  let update={distance:data.distance,runTime:data.runTime}
  console.log(update,data);
  RunDate.updateOne(wherestr, update, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
      res.send({status:200})
...
//查询数据
exports.findRunDate = (data, res)=>{
  let wherestr={'userID':data.id}
  RunDate.findOne(wherestr, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
      res.send({status:200, result})
...
//所有运动数据
exports.allRunDate = (data, res)=>{
  let query = RunDate.find({})
  if(data.class=='distance') query.sort({'distance':-1})
  else query.sort({'runTime':-1})
  // 条数
  query.limit(data.pagesize)
  // 查询结果
  query.exec().then( function (e){
...
exports.newDynamic = (data, res) => {
  ...
  // console.log(datas)
  let dynamic = new Dynamic(datas)
  // 保存
  dynamic.save((err, result)=>{
    if(err) {
      res.send({status:500})
      console.log(err)
  }
...
//删
exports.deleteDynamic = (data, res)=>{
  let wherestr = { _id:data.id}
  Dynamic.findByIdAndRemove(wherestr, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
      res.send({status:200})
...
//查
exports.findDynamic = (data, res)=>{
  let wherestr={'_id':data.id}
  Dynamic.findOne(wherestr, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
      res.send({status:200, result})
 ...
 * @param {*} res 
 */
 exports.DynamicLoad = (data, res) =>{
  
  let skipNum = (data.nowPage)*data.pageSize  // 跳过的条数

  let query = Dynamic.find({})
...
exports.newDiscuss = (data, res) => {
  ...
  console.log(datas)
  let discuss = new Discuss(datas)
  // 保存
  discuss.save((err, result)=>{
    if(err) {
      res.send({status:500})
      console.log(err)
  }
...
//删
exports.deleteDiscuss = (data, res)=>{
  let wherestr = { _id:data.id}
  Discuss.findByIdAndRemove(wherestr, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
      res.send({status:200})
...
//评论加载
exports.DiscussLoad = (data, res) =>{
  
  let skipNum = (data.nowPage)*data.pageSize  // 跳过的条数

  let query = Discuss.find({})

  query.where( { dynamicID:data.id } )
...
exports.getMessage = (data, res)=>{
  let wherestr
  let out = {'_id':1,'userID':1,'friendID':1, 'message': 1, 'types': 1,'time': 1}
  if(data === ''){
    wherestr = {}
    Message.find(wherestr,out, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
        res.send({status:200, result})
    ...
    // 模糊搜索
    wherestr = { $or:[{'message':{$regex:data}}]}
    Message.find(wherestr, out, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
...
//删
exports.delMessage = (data, res)=>{
  let wherestr = { _id:data._id}
  Message.findByIdAndRemove(wherestr, (err, result)=>{
    if(err){
      res.send({status:500})
    } else {
      res.send({status:200})
...
// 评论管理
exports.getDiscuss = (data, res)=>{
  let wherestr
  if(data === ''){
    wherestr = {}
    Discuss.find(wherestr, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
        res.send({status:200, result})
    ...
    // 模糊搜索
    wherestr = { $or:[{'message':{$regex:data}},{'name':{$regex:data}}]}
    Discuss.find(wherestr, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
...
// 动态管理
exports.getDynamic = (data, res)=>{
  let wherestr
  if(data === ''){
    wherestr = {}
    Dynamic.find(wherestr, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
        res.send({status:200, result})
...
exports.runDate = (data, res)=>{
    wherestr = {}
    // console.log(data);
    RunDate.find(wherestr, (err, result)=>{
      if(err){
        res.send({status:500})
      } else {
        res.send({status:200, result})
