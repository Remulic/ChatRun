<template>
	<view class="bottom">
	  <view class="nav-box" >
	    ...
	    <view class="nav-tab"
	          :class="[pageIndex==index?'nav-tab-active':'nav-tab-normal']"
	          v-for="(item,index) in list"
	          :key="index"
	          @click="toIndex(index)">
	      <view class="nav-icon">
	        <view class="circle">
	          <image :src="pageIndex==index?item.selectedIconPath:item.iconPath"></image>
	        </view>
	      </view> 
	      <view class="nav-text">
	        {{item.text}}
		...
		data() {
			return {
				list: [
					...
					{
						"iconPath": "/static/images/footbar/run2.png",
...
<style lang="scss">
$navHeight:100rpx; //导航栏高度
$navBoxHeight:50rpx; //导航栏盒子高度，XR系列底部HOME健
.bottom {
  position: fixed;
  bottom: 0;
  left: 0;
